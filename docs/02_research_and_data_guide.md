...
- 将 `_10 / _30 / _60 / _180_days` 等期限变体折叠为同一字段族，每个字段族默认先取不超过 2 个代表字段
- 在已有历史反馈时，默认保留约 40% 名额给未探索字段，避免偶然赢家长期占满候选池

人工传入 `--include-fields-file` 时视为明确研究选择，不再执行字段族配额与探索比例，但仍保留基础质量检查。运行 `--dry-run-plan` 可以直接查看字段分数、原始排名、字段族、入选原因和过滤统计。
