...
如果补全官方时间语义，完整生命周期是
`Simulate（5 年 IS）-> Check submission -> Submit -> Semi-OS -> OS`：

- `Semi-OS` 是 `IS` 结束到实际提交之间的过渡区间
- `OS` 是提交之后滚动积累的样本外表现
- 每个阶段的完整定义、状态与 `N/A` 语义见
  [04 的 IS/OS 与状态词典](04_platform_reference.md#2-最核心的时间维度is-和-os)
