...
| 官网区域 | 本地覆盖 | 边界 |
|---|---|---|
| Learn Documentation | `2026-08-06` 已重新保存 `24/24` 篇正文、元数据和官方 URL；其中 `15` 篇来自 Learn API，`9` 篇来自 Chrome 登录后的官网渲染页面；目录无新增或删除。`2026-08-13` 已登录浏览器复核：`24/24` 篇正文内容无变化，仅 `Introduction to BRAIN Expression Language` 从 Discover BRAIN 移到 Create Alphas（目录级调整） | `01-05` 只整合会改变研究、平台判断或操作流程的内容；完整例子和基础材料仍以原始快照为证据，不在主文档重复全文 |
...
| Operators | 最新目录保留了 `2026-08-03` 当前账号可见的 `7` 类、`66` 个 base operators 及签名说明；`2026-08-06` API 返回 `401`，因此明确标记为历史数据回退 | 回退目录不能证明算子列表在 `2026-08-06` 未变化；实际可用性仍以当前页面/API 和模拟结果为准 |
