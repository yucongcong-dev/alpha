...
def refresh_runtime_feedback(
    ...
    *,
    ...
    feedback = template_build_ctx.feedback
    cached_count = feedback.feedback_result_count
    if force:
        feedback.field_feedback = compile_field_feedback(results)
        feedback.global_failed_check_counts = compile_global_failed_check_counts(results)
        feedback.failed_check_counts_by_field_type = compile_failed_check_counts_by_field_type(
            results
        )
        feedback.feedback_result_count = result_count
    ...
    for result in results[cached_count:]:
        ...
        update_field_feedback_with_result(feedback.field_feedback, result)
        update_global_failed_check_counts_with_result(
            feedback.global_failed_check_counts,
            result,
        )
        update_failed_check_counts_by_field_type(
            feedback.failed_check_counts_by_field_type,
            result,
        )
    feedback.feedback_result_count = result_count
