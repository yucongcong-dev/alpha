...
from ..analysis.template_registry_rules import normalize_activation_scope, normalize_template_role
...
def choose_registry_settings_budget(
    ...
    *,
    ...
    if recommended_role == "promoted_core" or recommended_scope == "broad":
        return budget + 1 if feedback_stage == FEEDBACK_STAGE_RESIMULATE else budget
...
def choose_field_cluster_settings_budget(
    ...
    *,
    ...
    if not isinstance(cluster_overrides, Mapping):
        cluster_overrides = {}
    for tag in tags:
        if tag == "high_coverage" and feedback_stage == FEEDBACK_STAGE_RESIMULATE:
            budget += 1
