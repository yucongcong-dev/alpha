...
def test_build_payload_and_settings_fingerprint_use_resolved_values() -> None:
    args = _args(decay=8, start_date="2021-01-01", end_date="2021-12-31")
    payload = payload_module.build_simulation_payload(args, "rank(field)")
    other = payload_module.build_simulation_payload(args, "-rank(field)")
    ...
    assert payload_module.build_settings_fingerprint(args) == payload_module.stable_fingerprint(
        payload["settings"]
    )
