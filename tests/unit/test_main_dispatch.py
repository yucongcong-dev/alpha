...
from alpha.app import bootstrap, finalize, planning, run_lock, run_loop, submission_check_refresh
from alpha.cli import parser
...
def _disable_logging_setup(monkeypatch) -> None:
    ...
    monkeypatch.setattr(run_lock, "exclusive_run_lock", lambda _path: nullcontext())
...
def test_main_routes_dry_run_around_runtime_bootstrap_and_finalize(monkeypatch) -> None:
    ...
    monkeypatch.setattr(parser, "parse_application_config", lambda: config)
    monkeypatch.setattr(planning, "run_dry_run_plan", lambda args: True)

    def _unexpected(*_args, **_kwargs):
        raise AssertionError("runtime bootstrap/finalize must not run for a dry-run plan")

    monkeypatch.setattr(bootstrap, "initialize_run_context", _unexpected)
    monkeypatch.setattr(run_loop, "run_field_test_loop", _unexpected)
    monkeypatch.setattr(finalize, "finalize_run", _unexpected)
    monkeypatch.setattr(run_lock, "exclusive_run_lock", _unexpected)
...
def test_main_routes_clean_before_run_logging_and_bootstrap(monkeypatch) -> None:
    ...
    monkeypatch.setattr(parser, "parse_application_config", lambda: config)
    monkeypatch.setattr(bootstrap, "clean_runtime_artifacts", lambda _config: 0)
...
def test_main_routes_check_submissions_around_runtime_pipeline(monkeypatch) -> None:
    ...
    monkeypatch.setattr(parser, "parse_application_config", lambda: config)
    monkeypatch.setattr(main_module, "_configure_application_logging", lambda _config: None)
    monkeypatch.setattr(run_lock, "exclusive_run_lock", lambda _path: RecordingLock())
    monkeypatch.setattr(
        submission_check_refresh,
...
def test_main_closes_client_factory_when_runtime_pipeline_fails(monkeypatch) -> None:
    ...
    monkeypatch.setattr(run_loop, "run_field_test_loop", _fail_run)
    monkeypatch.setattr(
        finalize,
        "finalize_run",
